...
const SERVER_INSTRUCTIONS = [
  ...
  '- Several documented endpoints return 501 here, and two answer 200 with the wrong body; get_hc3_api_notes({topic:"dead_endpoints"}) lists them. Prefer a tool over a hand-built path.',
  '- **A call that does not throw has not necessarily worked.** HC3 stores requests it will not act on, and reports success: a QuickApp `select` missing `selectionType` is stored, reported verified, then blanks the **entire** tile. Read back what you care about, not the return code — and note a read-back proves the RECORD, not the effect: a Z-Wave parameter PUT caches without transmitting, so the record agrees with you while the node never hears about it. Every known instance, and where read-back stops helping: get_hc3_api_notes({topic:"silent_writes"}).',
