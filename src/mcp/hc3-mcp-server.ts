...
const SERVER_INSTRUCTIONS = [
  ...
  '- Bodies are large. get_scenes returns every scene in full (~1.9 MB) — use get_scene. Edit a QuickApp file with patch_quickapp_file, which sends only the change and refuses one that no longer fits, not by rewriting it whole.',
  '- Several documented endpoints return 501 on this firmware; KNOWN_DEAD_ENDPOINTS.md lists them. Prefer a tool over a hand-built path.',
  ...
  '- Clients cache tool schemas at connect. After a redeploy, reconnect the session or you are calling yesterday\'s API.',
  '',
