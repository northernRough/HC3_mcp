...
- **The `properties.icon` external-write claim was stronger than its
  evidence.** Both the `ui` guide topic and the silent-write catalogue stated
  flatly that an external `api.put` of `properties.icon` is "accepted and
  silently discarded". Probed on an idle throwaway QuickApp it PERSISTS, so the
  endpoint is not what drops it — the likelier mechanism is the QuickApp
  overwriting the property when it starts, and every external write restarts
  it. The field report that produced the claim involved a running QA; the probe
  did not. Now stated as conditional on the QA running, with the untested part
  named. This is the only change in this release that reaches an installed
  package.

### Maintainer tooling — not published

`scripts/` is not in the package `files` array, so none of the following ships
to npm. It reaches a git checkout and the Pi deployment, which is where it is
used.

- `npm run firmware-check` — a firmware-change differ. Records the gateway's
  behaviour per firmware version, and on the next run against a different
  version reports what BROKE, what was FIXED, and what merely CHANGED. Not a
  monitor: firmware here does not arrive on a schedule.
  - Walks `KNOWN_DEAD_ENDPOINTS.md` itself rather than a copied list, and
    reports where the gateway now disagrees with it. This is the tier that
    catches an endpoint coming back to life, which that document explicitly
    warns can happen across upgrades.
  - `--with-writes` re-tests the silent-write catalogue on throwaway objects,
    so a firmware that quietly FIXES a silent write shows up as good news. A
    read-only check can never see that.
- `scripts/test/shape.mjs` — a structural signature that changes when the
  ...
  of fifty-eight tools "drifted" in one ordinary day.
- **The live test phases could not authenticate.** `mcp-client.mjs` spawned the
  server with a bare `process.env`, which carries no `FIBARO_*` on a normal
  shell, so every live phase reported "Fibaro HC3 not configured" and 53 of 58
  tools recorded an error. It read as a broken tier for months; it was a
  missing environment. The read-only sweep goes from 5/58 to 56/58.
- `withQuickApp` accepts `initialView` / `initialProperties` / `roomId`, because
  several HC3 behaviours only occur AT CREATION and a probe that creates a bare
  QA then PUTs to it tests a different path.
